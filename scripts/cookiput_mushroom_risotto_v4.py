#!/usr/bin/env python3
"""
Final version of cookiput script with proper recipe ID extraction.
"""
    ...
    def create_recipe_stub(self, recipe_name: str) -> str:
        ...
        try:
            ...
            if response.status_code == 200 or response.status_code == 201:
                data = response.json()
                # Try different keys for recipe ID
                recipe_id = data.get('recipeId') or data.get('id') or data.get('recipe_id')
                if not recipe_id:
                    # Parse from URL if present in response
                    location_header = response.headers.get('Location')
                    if location_header:
                        # Extract ID from URL like /created-recipes/de-DE/recipeId
